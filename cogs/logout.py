...
class logout(commands.Cog):
    ...
    async def on_ready(self):
        ...
        await channel.send(f"logout load {datetime.datetime.now(pytz.timezone('Asia/Yekaterinburg')).strftime('%d-%m-%Y at %H:%M:%S')}")

    @commands.command(name="exit", aliases=["выйти"])
